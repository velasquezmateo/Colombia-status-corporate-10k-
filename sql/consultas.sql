...
from tabla_hechos f
inner join macrosector m on f.id_macrosector=m.id_macrosector
inner join geografia g on f.id_ciudad=g.id_ciudad
GROUP BY g.departamento_domicilio, m.Macrosector) as subconsulta
where Ranking <=3;
...
(select e.Razon_social, f.Ganancia_perdida, a.Anio_de_corte,
lag(f.Ganancia_perdida) over (partition by e.Razon_social order by a.Anio_de_corte asc) as ganancia_anterior
from tabla_hechos f
...
f.Total_patrimonio,
a.Anio_de_corte,
lag(f.Total_patrimonio) over (partition by e.Razon_social order by a.Anio_de_corte asc) as patrimonio_anterior
from tabla_hechos f
left join empresas e on f.id_empresa=e.id_empresa
left join anio_corte a on f.id_anio=a.id_anio) as tabla
...
a.Anio_de_corte, 
lag(f.Ingresos_operacionales) over (partition by m.Macrosector, e.Razon_social order by a.Anio_de_corte) as Ingresos_año_anterior
from tabla_hechos f
...
tabla2 as(
select Razon_social, Macrosector, ((Ingresos_operacionales-Ingresos_año_anterior)/Ingresos_año_anterior)*100 as 'Tasa crecimiento (%)', 
	Anio_de_corte,
...
m.Macrosector,
f.Ingresos_operacionales,
lag(f.Ingresos_operacionales) over (partition by e.Razon_social order by a.Anio_de_corte asc) as ingresos_año_anterior, 
f.Total_patrimonio,
f.ROE,
a.Anio_de_corte
from tabla_hechos f  
...
select *, concat(round((Ingresos_2024/Ingresos_2024_region)*100,2),'%') as 'Dominancia',
rank() over (partition by departamento_domicilio,Macrosector order by ((Ingresos_2024/Ingresos_2024_region)*100) desc) as 'Ranking' 
from
(select e.Razon_social, 
m.Macrosector,
